...
void rotacionar90(int *dest, int *orig, int n){ // Rotaciona a matriz 90 graus no sentido horário
    for(int i=0; i<n; i++){
        dest[n-1 - orig[i]] = i;
    }
    return;
...
void refletir(int *dest, int *orig, int n){ // Reflete a matriz horizontalmente
    for(int i=0; i<n; i++){
        dest[n-1 - i] = orig[i];
    }
    return;
...
int igual(int *a, int *b, int n){ // Verifica se os dois vetores são iguais
    for(int i=0; i<n; i++){
        if(a[i] != b[i]){
            return 0;
        }
...
int verificar_solucao(int *rainhas, int n){ // Para verificar as equivalencias basta verificar as rotações (0°, 90°, 180°, 270°) e fazer uma reflexão de cada
    int temp[n], rot[n], ref[n];
    memcpy(temp, rainhas, n*sizeof(int));
    for(int i=0; i<4; i++){
        for(int j=0; j<totalSolucoes; j++){
            if(igual(temp, solucoes[j].pos, n)) return 0; // Já existe uma solução equivalente
        }
        refletir(ref, temp, n);
        for(int j=0; j<totalSolucoes; j++){
            if(igual(ref, solucoes[j].pos, n)) return 0; // Já existe uma solução equivalente
        }
...
int resolver(int n, int col, int *rainhas, int *linUsada, int *diag1Usada, int *diag2Usada) {
    ...
    if (col == n) { // Solução encontrada
        verificar_solucao(rainhas, n);
        return 1; 
    }

    for (int lin = 0; lin < n; lin++) {
        if(linUsada[lin] == 0 && diag1Usada[lin - col + n] == 0 && diag2Usada[lin + col] == 0){ // Se a linha e as diagonais não estão ocupadas
